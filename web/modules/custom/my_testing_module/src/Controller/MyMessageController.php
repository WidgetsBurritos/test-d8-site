...
class MyMessageController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function getMessageForUser(AccountInterface $user) {
    ...
    if ($user->hasPermission('my super secret privilege')) {
      $this->logUsers && $this->logger->info('super secret privilege granted for @user', ['@user' => $user->getDisplayName()]);
      $has_elevated_permission = TRUE;
      $messages[] = $this->t("You aren't all that special.");
    }
    if ($user->hasPermission('yet another privilege')) {
      $this->logUsers && $this->logger->info('yet another privilege granted for @user', ['@user' => $user->getDisplayName()]);
      $has_elevated_permission = TRUE;
      $messages[] = $this->t('You have yet another privilege.');
    }
    if (!$has_elevated_permission) {
      $this->logUsers && $this->logger->warning('unprivileged access for @user', ['@user' => $user->getDisplayName()]);
    }
